...
class LayrzTreeRowStyleSpec {
  ...
  /// hover/selected/active states — this factory is the single place that
  /// rule is applied for tree rows. [isActive] marks the row currently
  ...
  /// row to stay legible against a background that can change per-row. The
  /// resting fill is `sf1` -- the app's own base surface step, per DESIGN-171
  /// -- rather than a literal transparent fill, so it never seams against an
  /// enclosing container that also paints `sf1` while still resolving to a
  /// single, testable token instead of ad hoc alpha blending.
