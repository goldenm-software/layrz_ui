...
/// [LayrzComboBoxInput], [LayrzMultiSelectInput], and [LayrzDualListInput]. It is intentionally
/// reduced to exactly three fields: a typed [value], a set of [searchableStrings] used purely
/// for filtering, and a [child] widget that is the item's **only** presentation.
///
/// **BREAKING:** earlier versions also carried a `labelText` string that served double duty as
/// both the default display text and the primary search key. That field is gone. The item now
/// defines its own presentation entirely through [child] -- in selection surfaces (dropdowns,
/// bottom sheets) and, where the consuming widget supports it, in the field itself while idle.
/// A caller that put a flag icon next to a country name in [child] sees that same flag in the
/// field, not a degraded plain-text fallback. Search is a fully separate concern, driven only
/// by [searchableStrings] -- so an item can be found by text that never appears on screen at all
/// (an ID, an alternate spelling, a category tag).
@immutable
class LayrzSelectItem<T> {
  ...
  ///
  /// Rendered in selection surfaces (dropdowns, dialogs, bottom sheets) in place of this
  /// item's row content, and, in widgets that support it (e.g. [LayrzSelectInput]), in the
  /// field itself while idle with this item selected. There is no separate label string
  /// anywhere in this type: whatever [child] renders is what the user sees, everywhere this
  /// item appears. Required -- an item with nothing to render is not a valid item.
  final Widget child;
