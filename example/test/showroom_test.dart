...
void main() {
  group('Showroom', () {
    ...
    testWidgets('hover animation does not change container size', (WidgetTester tester) async {
      ...
      // Trigger hover using the proven idiom from tooltip tests:
      // createGesture already registers the pointer; just moveTo the target.
      // Do NOT call addPointer — that bookkeeping breaks enter/exit events.
      final mouse = await tester.createGesture(kind: PointerDeviceKind.mouse);
      addTearDown(mouse.removePointer);

      final containerCenter = tester.getCenter(hoverDemoFinder);
      await mouse.moveTo(containerCenter);
