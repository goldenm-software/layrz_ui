...
class _TableSectionState extends State<TableSection> {
  ...
  /// The number of sample rows generated by [_buildVehicles].
  ///
  /// Set to 30,000 to mirror the real production scenario for these tables —
  /// large enough to exercise sort, search, and virtualized scrolling under a
  /// realistic dataset size rather than the small illustrative set used
  /// before.
  static const int _rowCount = 30000;
  ...
  /// Generates [_rowCount] sample vehicles covering a spread of statuses,
  /// odometer readings, service dates, and idle durations.
  ///
  /// Runs once (from [initState]) and is held in [_vehicles] for the life of
  /// this state — it must never be recomputed inside [build]. Every field is
  /// derived deterministically from `index` by cycling through a small set of
  /// sample names/statuses, so generation stays cheap (a single linear pass,
  /// no allocation beyond the resulting list) even at 30,000 rows.
