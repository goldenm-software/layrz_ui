...
- **`LayrzButtonGroup`** — Horizontal row of `LayrzButton` actions that collapses below the `md` breakpoint into a single fab trigger opening a `LayrzDropdownMenu`. Mode driven by a nullable `bool useDropdown` parameter; no enum exposed. Replaces the old `ThemedActionsButtons` from layrz_theme. Renamed from `LayrzGroupedButton` for uniformity with `LayrzChipGroup` (group components follow `Layrz<Thing>Group` naming). The `triggerHintText` parameter is required and serves as the trigger's stable accessible name (tooltip and screen reader label), not derived from action contents. Platform overflow menus identify the control by name, not enumerate their actions. Includes 27 tests covering row mode, dropdown mode, responsive switching, semantic type preservation, disabled states, and accessibility. Reference DESIGN-31.

- **`LayrzButtonGroup.builder`** — Variant constructor allowing callers to supply a custom trigger widget instead of the default `LayrzButtonStyle.elevatedFab`. The builder receives the `MenuController` and must wire it to the trigger's own tap handler. Enables different button styles, icons, labels, or entirely custom widgets as the trigger. Row mode is identical for both constructors; the builder is only consulted in dropdown mode. Gesture-arena warning: do not wrap the trigger in `GestureDetector`, as `LayrzButton`'s non-null `onTapCancel` wins the arena and prevents the menu from opening. Wire the controller directly to the trigger's own `onTap`. Includes 7 tests covering custom styling, menu toggle, action callbacks, row mode, breakpoint responsiveness, and empty actions. Mirrors the `LayrzDropdownMenu` builder pattern for API familiarity.
...
- **`LayrzAvatar`** — Static display component rendering a layrz_sdk `Avatar` by type (photo, initials, icon), with optional initials fallback. Always a rounded box using the `r12` radius token, consistent with `LayrzCard` and `LayrzAlert`, and carries a fixed `tokens.shadow.compact1` drop shadow in all render modes. No interaction affordances; callers wrap if needed. Initials algorithm is deterministic but not locale-aware (no Unicode segmentation). Reference decision D31.
