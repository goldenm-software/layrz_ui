...
- **Behavior change — `LayrzConnectionIndicator.full` mode is now a self-contained chip** that displays the localized connection state label ("Online", "Idle", "Offline", "Disconnected", "No data") on the state-colored chip, instead of wrapping caller content. Any `child` passed in `.full` mode is now ignored — `child` is used only by `.dot` mode, as the badge host. The old `.full ⇒ child != null` constructor assertion is removed; `child` is optional in both modes now.
...
**On web, both fields render a real HTML `<input>` via a platform view** so browser password managers can detect and offer to fill the credential pair. This is a best-effort measure: Flutter's web engine stamps `aria-hidden` on the platform-view wrapper (flutter/flutter#105485), the accessibility channel managers read, so detection is worked around but not guaranteed — and the workaround holds only while Flutter accessibility semantics is off. The web chrome is a CSS re-creation reading the same `LayrzInputStyleSpec` and `LayrzTokens` the native `LayrzInputChrome` uses, so rest/hover/focus/error/disabled states, the primary focus border, the danger error border (1.5px, per decision D15 — colour changes by state, geometry does not) and a right-aligned error alert icon all match the native field. Icons tint by state (primary on focus, danger on error). `Ctrl`/`Cmd`+`A` selects the field's own text rather than the page. The `login/web/` DOM layer is internal to these two widgets — it is not exported and is not a general web-input primitive.

**Password strength now follows `layrz_theme`'s exact rules, shown as physical elements below the field instead of a tooltip.** `LayrzPasswordRequirements.evaluate(password)` exposes the four requirement checks (lowercase, uppercase, digit, special character), an allowed-character check, validity, and a 0–4 strength `level` computed from length once valid (`<8`→0, `<12`→1, `<16`→2, `<20`→3, `>=20`→4). `LayrzPasswordStrengthMeter` renders a full-width four-segment bar (filled to the level, coloured danger → warning → success) with a requirements checklist beneath it. It is opt-in via `LayrzPasswordInput.showStrengthMeter` and localised through the existing `LayrzUiL10nPasswordMixin` keys.

**Custom fonts now render in web DOM content.** `LayrzFont` gains an optional `registerOnWeb()` hook (no-op by default) that registers a browser `@font-face` so DOM-rendered content — the web login `<input>` above, and anything similar in future — uses the theme's font instead of falling back to `sans-serif`. Two helpers back it: `registerWebFont({family, url})` for URL/CDN-hosted fonts and `registerWebFontFromBytes({family, bytes})` for bundled assets loaded via `rootBundle`. Registration is fired automatically from the theme pipeline (`LayrzTextTheme.defaults`), so any app that passes a font to `LayrzThemeData.light(font:)` is covered with no code change; the actual `package:web` call sits behind a conditional-export seam so it never reaches a native build. `LayrzFont.load()` (Flutter engine) and `registerOnWeb()` (browser DOM) remain separate concerns.

- Added `web` and `pointer_interceptor` dependencies (web-targeted; gated so non-web builds never pull them).
- Added six localisation keys to `LayrzUiL10nPasswordMixin` for the eye-toggle label/state announcements and the login field labels.

## 0.0.18

**`LayrzSnackbar` is a new public component** (`lib/src/snackbar/`) — the Material-free, Cupertino-free replacement for `layrz_theme`'s `ThemedSnackbar`. It is a white card whose icon and title take the semantic (or `custom`) colour while the description stays neutral grey, with an auto-dismiss progress bar along the top edge. The constructor takes `titleText` and `descriptionText` (both required), a `type` severity enum defaulting to `success` (`success`, `danger`, `warning`, `info`, `context`, `custom`), `icon` and `color` (valid and required only when `type` is `custom`, asserted null otherwise in both directions), an optional `onTap` that runs an action and dismisses, and `actions` — a `List<LayrzButton>` rendered below the content (strictly `LayrzButton`). Both the close affordance and every action are `LayrzButton`s.

**Dismissal is driven entirely by `duration`.** `duration` is a `Duration?` defaulting to `Duration(seconds: 10)`: a non-null duration auto-dismisses and shows the progress bar plus a close button, while **`duration: null` makes the snackbar persistent** — no bar, no timer, no close button, removed only programmatically via `dismissAll()`, an action, or its `onTap`. Convenience getters `isPersistent` and `isAutoDismiss` expose the derived state. **BREAKING vs. `ThemedSnackbar`:** `descriptionText` is the renamed `message`, `type` uses `.danger` (not `.error`), and there is no `isDismissible` field — dismissibility is inferred from `duration`.

**`LayrzSnackbarMessenger` hosts the toasts and is installed automatically by `LayrzApp`** (both `LayrzApp` and `LayrzApp.router`), so consumers never wire it and never touch `builder:`; a snackbar is shown with `LayrzSnackbarMessenger.of(context).show(LayrzSnackbar(...))`. There is deliberately **no `GlobalKey` and no `showGlobal`** — access is by tree ancestry only, the same context-based path `layrz_theme` moved to after its global-key messenger proved troublesome. Toasts stack as an accordion: a compact deck at rest (newest on top, older cards peeking a slim sliver below and slightly faded) that fans out downward on hover to reveal every card's full content and pauses the auto-dismiss timer while hovered, with a configurable `maxVisible` (default 3) collapsing the overflow into a "+N · Dismiss all" chip. The card announces its title and description together as a live region and the close control carries a ≥44-logical-pixel touch target.

- Fixed an overflow in `InputsSection` of the example showroom by wrapping its `Column` in a `Flexible`.

## 0.0.17

**The complete M4 date/time picker family lands: `LayrzDateInput`, `LayrzDateRangeInput`, `LayrzTimeInput`, `LayrzTimeRangeInput`, `LayrzDateTimeInput`, `LayrzDateTimeRangeInput`, `LayrzMonthInput` and `LayrzMonthRangeInput`** — eight `Layrz*Input` widgets replacing the retired `Themed*Picker` set from `layrz_theme`. Dates use a compact Monday-first calendar with an ISO week-number gutter (`showWeekNumbers`), greyed adjacent-month days and a `<` `>` month header. Time is entered as fields, never a clock: HH / MM with an optional seconds field (`showSeconds`), no interval snapping, out-of-range input clamped rather than dropped, 24h by default with 12h supported. Months render as a 4-column by 3-row grid of full names under `<` `Year YYYY` `>` navigation. Range variants are contiguous-only and reajustable — re-tapping an endpoint moves that edge while the other stays fixed — except `LayrzMonthRangeInput`, the one widget that allows non-consecutive selection (`consecutive: false` by default).

New public value types back these widgets: `LayrzMonth`, `LayrzTimeOfDay` (with seconds), and `LayrzDateRange` / `LayrzMonthRange` with non-nullable `start`/`end`, replacing the old `List<DateTime>` shape guarded by a runtime `assert(length == 0 || 2)`. A public `formatStrftime(DateTime, String pattern, LayrzUiL10n)` helper ships in the new `lib/src/formatting/` module, using Python `strftime` directives (`%Y %m %d %H %I %M %S %B %b %A %a %p`) with no `intl` dependency — the same pattern style the old `layrz_theme` pickers used, so existing format strings carry over. Incoming `TZDateTime` values preserve their zone via the newly exported `sameZoneDate` / `sameZoneDateTime`.

**`LayrzEndDrawer` is a new public component** (`lib/src/sheets/`): a right-edge, fixed-width (420 logical pixels) drawer built on `LayrzModalRoute`, with a bottom-pinned `actions` slot and a `title` slot, and a `canDismiss` inference matching `LayrzDialog`/`LayrzBottomSheet`. On desktop it is the container for every date/time input plus `LayrzDurationInput`, `LayrzSelectInput` and `LayrzComboBoxInput`; below 960 logical pixels these fall back to `LayrzBottomSheet`. `LayrzSearchInput` deliberately keeps the anchored panel. As a consequence, the commit model changed: all eight pickers now carry an in-drawer `Save` rather than committing on tap, and midnight (`00:00`) and `Duration.zero` are valid, committable values — `canSave` gates on the date, not on whether a time was touched.

`LayrzDateTimeInputPresentation` (`tabbed` / `stepped`) is deprecated and inert — the drawer shows the calendar and time fields together, so neither value has any effect; the parameter is retained to avoid breaking callers and can be dropped. `LayrzComboBoxInput` regains its custom-value affordance as a bold first row (weight is the sole indicator, no "custom" label) with keyboard navigation rebuilt inside the drawer, and its option list opens unfiltered. Range selections render as a flat `primary` connected bar with no per-cell circles or cards. Grid navigation is keyboard-accessible (WCAG 2.1.1 Level A) with a visible focus ring, and every drawer surface shows its `labelText` as the drawer title.
