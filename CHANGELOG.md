...
**`LayrzSelectItem<T>` reduced to exactly three fields — `value`, `child`, `searchableStrings` (BREAKING).** `labelText` is gone entirely, `searchableAttributes` is renamed `searchableStrings`, and `child` is now required: it is the item's only presentation, everywhere the item is rendered, including `LayrzSelectInput`'s own field while idle (previously the field showed only a plain label; now it renders the selected item's `child` widget directly — icon, formatting, and all). Search is driven solely by `searchableStrings`, with no `labelText` fallback, so an item can be matched by text that never appears on screen at all. Also fixes a real "white text" bug (`item.child` was rendered with no explicit `DefaultTextStyle`, so a plain `Text` inside it could resolve to a null color, which the engine then paints solid white) by wrapping `child` in the theme's body style at every render site. See the Breaking section for the full migration note, including the `RichText`-vs-`Text.rich` trap this reduction introduces and the radio accessibility gap for icon-only items.
...
**`LayrzSelectItem<T>` reduced to exactly three fields (DESIGN-142) — used by `LayrzSelectInput`, `LayrzRadioInput`, `LayrzComboBoxInput`, `LayrzMultiSelectInput`, and `LayrzDualListInput`:**
- **`labelText` is removed entirely.** There is no string field on `LayrzSelectItem<T>` anymore.
- **`child` is now `required`** (previously optional). It is the item's only presentation — in every selection surface (dropdown list, bottom sheet) and, in `LayrzSelectInput` specifically, in the field itself while idle with that item selected. A caller migrating from `labelText: 'Foo'` supplies `child: const Text('Foo')` instead.
- **`searchableAttributes` is renamed `searchableStrings`**, same `Set<String>` shape and case-insensitive substring-matching semantics. It is now the **only** thing `LayrzSelectItem.matches` checks — there is no `labelText` fallback anymore. A caller that wants an item's visible text to also be searchable must add it to `searchableStrings` explicitly; it is no longer implied by whatever `child` happens to render.
- **`LayrzSelectInput`'s field now renders the selected item's `child` while idle**, not a label string. Mode 1 (idle) and mode 3 (blur-revert) now show the actual `child` widget; mode 2 (typing) and mode 4 (external reconcile mid-query) are unaffected — the field's `EditableText` still carries the live query exactly as before.
- **Fixes a real "white text" bug.** `item.child` was previously rendered bare, with no `DefaultTextStyle` of its own — it depended entirely on whatever `DefaultTextStyle` happened to be ambient at its mount point. A plain `Text` inside `child` with no explicit color resolves `style.color` to `null` when there is no real ancestor supplying one, and the rendering engine then paints that as solid white rather than the theme's body color (verified by pixel-sampling the render). `item.child` is now force-wrapped in `DefaultTextStyle(style: tokens.typography.body)` at every render site (the surface's list row, `LayrzRadioInput`'s option row, and `LayrzSelectInput`'s field-idle display).
- **New trap this reduction introduces: `RichText` does not inherit `DefaultTextStyle`; `Text` and `Text.rich` do.** Since `child` is now the item's only presentation, a `child` built from the raw `RichText` widget renders its `TextSpan`'s own style only — with no explicit color, that is the same null-color-renders-white failure described above, and the forced `DefaultTextStyle` wrap above cannot reach it (it only reaches widgets that actually inherit the ambient style). Use `Text.rich` instead of `RichText` whenever a `child` needs multiple styled runs (e.g. an icon glyph followed by a name) — identical `TextSpan` API, but it applies the ambient style the way a plain `Text` does. The showroom's select demo (`Custom Item Child` section) now demonstrates this pattern.
- **`LayrzRadioInput` accessibility migration note.** Previously, each option announced `LayrzSelectItem.labelText` via an explicit `Semantics(label: ...)`, with the presentation widget excluded from semantics (`ExcludeSemantics`) to avoid a double announcement. With `labelText` gone, both the explicit label and the exclusion are removed: the option's `child` is left un-excluded, and its own semantics (a plain `Text`, in the common case) merge upward into the same node instead — producing an identical single announcement, verified directly. **A `child` with no inherent text semantics (icon-only, a color swatch, etc.) now announces with no accessible name at all**, unless the caller wraps that `child` in its own `Semantics(label: '...')`. No shipped demo or test has an icon-only radio item today, but this is a genuine reduction in default accessibility coverage for any future one.
- **Migration:** replace every `LayrzSelectItem(labelText: 'Foo', value: v)` with `LayrzSelectItem(value: v, child: const Text('Foo'))`; if that item's text should remain searchable, add `searchableStrings: {'Foo'}` too (previously implied by `labelText`, now not). Replace `searchableAttributes:` with `searchableStrings:` everywhere else. Audit any `child` built with `RichText` and switch it to `Text.rich`. Audit any icon-only `child` (radio or otherwise) that relied on `labelText` for its accessible name and add an explicit `Semantics(label:)` to that `child`.
