...
`LayrzCalendar` ships its **month view only**. `LayrzCalendarMode.week` and `.day` exist in the enum but throw `UnimplementedError`, and the mode switcher disables both — a loud failure was chosen deliberately over a silently blank surface. It renders disabled dates, single-day events, and multi-day events as one continuous bar per week row (a multi-day event is one bar with its label shown once, not one chip repeated in every cell it spans). Grid lines come from a `divider`-coloured container with each cell inset by `tokens.border.stroke1`, so every line is exactly one stroke wide — per-cell borders produced double-width interior lines against a single-width outer edge. Today's ring is kept as a deliberate accent, not a grid line. Month names are currently hardcoded English: no month-name l10n mixin exists yet, so `"August 2026"` and the day-cell semantics are not localizable.
